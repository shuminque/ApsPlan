...
public class MongoShiftScheduleSyncListener {
    ...
    public void startListener() {
        ...
        ChangeStreamRequest<Document> request = ChangeStreamRequest.builder(this::onMessage)
    ...
    private void onMessage(Message<ChangeStreamDocument<Document>, Document> message) {
        ...
        try {
            if (OperationType.DELETE.equals(operationType)) {
                String mongoId = extractMongoId(raw.getDocumentKey());
                if (mongoId != null) {
            ...
            log.error("Mongo -> MySQL sync failed. operationType={}, message={}", operationType, e.getMessage(), e);
        }
    }
    ...
    private ShiftSchedule convert(Document doc) {
        ...
        if (mongoId == null || date == null || day == null) {
            log.warn("Skip sync due to missing required fields. _id={}, date={}, day={}", mongoId, date, day);
            return null;
        }

        LocalDate scheduleDate;
        try {
            scheduleDate = LocalDate.parse(date + "-" + day, DateTimeFormatter.ofPattern("yyyy-MM-d"));
        ...
        schedule.setScheduleDate(Date.from(scheduleDate.atStartOfDay(ZoneId.systemDefault()).toInstant()));
